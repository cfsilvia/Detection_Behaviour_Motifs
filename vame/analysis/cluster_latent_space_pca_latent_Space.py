...
class cluster_latent_space_pca_latent_Space:
    def __init__(self,config,exp_file = None):
       ...
       self.video_file = os.path.join(self.project_path, "videos", self.file_exp[0] + '.avi') 
       self.cluster_start = cfg['time_window'] / 2
       self.data_file = os.path.join(self.project_path, "videos", "pose_estimation" ) 
       self.number_video = 0 # number video to check
 ...
 ############################################################      
    ...
    def write_movie(self, m, capture, frames_numbers,width, height, fps):
       ...
       output_path = os.path.join(self.save_data, 'pca' +'-'+str(self.n_cluster) + 
